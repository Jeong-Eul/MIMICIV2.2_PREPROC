...
if not os.path.exists(local+"/features/chartevents"):
    os.makedirs(local+"/features/chartevents")
  
def feature_icu(root_dir, cohort_output, version_path, diag_flag=True,out_flag=True,chart_flag=True,proc_flag=True,med_flag=True, ing_flag=True, lab_flag=True):
    if diag_flag:
    ...
    if proc_flag:
        ...
        proc = preproc_proc(root_dir+version_path+"/icu/procedureevents.csv.gz", local+'/cohort/'+cohort_output+'.csv.gz', 'starttime', dtypes=None, usecols=['stay_id','starttime','itemid'])
        proc[['subject_id', 'hadm_id', 'stay_id', 'itemid', 'starttime', 'intime', 'event_time_from_admit']].to_csv(local+"/features/preproc_proc_icu.csv.gz", compression='gzip', index=False)
...
def generate_summary_icu(local_dir , diag_flag,proc_flag,med_flag,out_flag,chart_flag,ing_flag,lab_flag):
...
def features_selection_icu(local_dir, cohort_output, diag_flag, proc_flag, med_flag, ing_flag, out_flag, lab_flag, chart_flag, group_diag, 

...
def feature_icu(cohort_output, version_path, diag_flag=True,out_flag=True,chart_flag=True,proc_flag=True,med_flag=True, ing_flag=True):
    if diag_flag:
...
def feature_icu_circ(cohort_output, version_path, diag_flag=True,out_flag=True,chart_flag=True,proc_flag=True,med_flag=True, ing_flag=True, lab_flag=True):
    if diag_flag:
        print("[EXTRACTING DIAGNOSIS DATA]")
        diag = preproc_icd_module("./"+version_path+"/hosp/diagnoses_icd.csv.gz", './data/cohort/'+cohort_output+'.csv.gz', './utils/mappings/ICD9_to_ICD10_mapping.txt', map_code_colname='diagnosis_code')
        diag[['subject_id', 'hadm_id', 'stay_id', 'icd_code','root_icd10_convert','root']].to_csv("./data/features/preproc_diag_icu.csv.gz", compression='gzip', index=False)
...
def generate_summary_icu(diag_flag,proc_flag,med_flag,out_flag,chart_flag, ing_flag):
...
def generate_summary_icu_circ(diag_flag,proc_flag,med_flag,out_flag,chart_flag,ing_flag,lab_flag):
    print("[GENERATING FEATURE SUMMARY]")
    if diag_flag:
        diag = pd.read_csv("./data/features/preproc_diag_icu.csv.gz", compression='gzip',header=0)
        freq=diag.groupby(['stay_id','new_icd_code']).size().reset_index(name="mean_frequency")
...
def features_selection_icu(cohort_output, diag_flag,proc_flag,med_flag,ing_flag,out_flag,lab_flag,chart_flag,group_diag,group_med,
    ...
    if ing_flag:       
        if group_med:   
            ...
            ing = pd.read_csv("./data/features/preproc_ing_icu.csv.gz", compression='gzip',header=0)
            features=pd.read_csv("./data/summary/ing_features.csv",header=0)
